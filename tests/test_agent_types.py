...
def test_create_type_rejects_empty_responsibility(services, project_with_owner) -> None:
    owner, project = project_with_owner
...
def test_create_instance_respects_concurrency_limit(services, project_with_owner) -> None:
    ...
    services.agent_types.assign_instance_to_task(
        project_id=project.id, instance_id=inst1.id, task_id=task.id, actor_id=owner.id
    )
    services.agent_types.assign_instance_to_task(
        project_id=project.id, instance_id=inst2.id, task_id=task.id, actor_id=owner.id
    )
    ...
    with pytest.raises(ConcurrencyLimitExceededError):
        services.agent_types.assign_instance_to_task(
            project_id=project.id, instance_id=inst3.id, task_id=task.id, actor_id=owner.id
        )


def test_cannot_create_instance_of_archived_type(services, project_with_owner) -> None:
    owner, project = project_with_owner
    t = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="T", responsibility="R", memory_scope="M"
    )
    # Archive the type via retire.
    services.agent_types.retire_type(project_id=project.id, type_id=t.id, actor_id=owner.id)
    with pytest.raises(InvalidAgentTypeTransitionError):
        services.agent_types.create_instance(project_id=project.id, type_id=t.id, actor_id=owner.id)
...
def test_add_knowledge_rejects_empty_content(services, project_with_owner) -> None:
    owner, project = project_with_owner
    t = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="T", responsibility="R", memory_scope="M"
    )
    with pytest.raises(ValueError, match="content"):
        services.agent_types.add_knowledge(
            project_id=project.id, type_id=t.id, actor_id=owner.id, kind="fact", content=""
        )


def test_list_knowledge_for_type_returns_only_owned_records(services, project_with_owner) -> None:
    """Per zero-project-isolation-evidence: knowledge is type-scoped."""
    owner, project = project_with_owner
    t1 = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="T1", responsibility="R1", memory_scope="M1"
    )
    t2 = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="T2", responsibility="R2", memory_scope="M2"
    )
    services.agent_types.add_knowledge(
        project_id=project.id, type_id=t1.id, actor_id=owner.id, kind="fact", content="T1 fact"
    )
    services.agent_types.add_knowledge(
        project_id=project.id, type_id=t2.id, actor_id=owner.id, kind="fact", content="T2 fact"
    )
    t1_records = services.agent_types.list_knowledge_for_type(project.id, t1.id, actor_id=owner.id)
    t2_records = services.agent_types.list_knowledge_for_type(project.id, t2.id, actor_id=owner.id)
...
def test_split_routes_knowledge_to_destinations(services, project_with_owner) -> None:
    ...
    runtime_records = services.agent_types.list_knowledge_for_type(
        project.id, runtime.id, actor_id=owner.id
    )
    ...
    adapter_records = services.agent_types.list_knowledge_for_type(
        project.id, adapters.id, actor_id=owner.id
    )
    assert any(r.id == r2.id for r in adapter_records)
    # r3 is archived (unrouted, not deleted).
    all_src = services.agent_types._repo.list_knowledge_for_type(src.id, include_archived=True)
...
def test_split_takes_topology_snapshot(services, project_with_owner) -> None:
    owner, project = project_with_owner
    src = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="Src", responsibility="R", memory_scope="M"
    )
...
def test_merge_migrates_all_knowledge_with_provenance(services, project_with_owner) -> None:
    ...
    services.agent_types.add_knowledge(
        project_id=project.id, type_id=t1.id, actor_id=owner.id, kind="fact", content="Uses React"
    )
    services.agent_types.add_knowledge(
        project_id=project.id, type_id=t2.id, actor_id=owner.id, kind="fact", content="Uses FastAPI"
    )
    merged, _snapshot = services.agent_types.merge_types(
        project_id=project.id,
        source_type_ids=[t1.id, t2.id],
        destination_name="Full Stack",
        destination_responsibility="End-to-end",
        destination_memory_scope="All",
        actor_id=owner.id,
    )
    records = services.agent_types.list_knowledge_for_type(project.id, merged.id, actor_id=owner.id)
...
def test_merge_requires_at_least_two_sources(services, project_with_owner) -> None:
    owner, project = project_with_owner
    t1 = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="T1", responsibility="R", memory_scope="M"
    )
    with pytest.raises(ValueError, match="at least 2"):
        services.agent_types.merge_types(
            project_id=project.id,
            source_type_ids=[t1.id],
...
def test_retire_archives_knowledge_never_deletes(services, project_with_owner) -> None:
    """Per zero-context-memory: removing a type never deletes its
    knowledge."""
    owner, project = project_with_owner
    t = services.agent_types.create_type(
        project_id=project.id, actor_id=owner.id, name="Temp", responsibility="R", memory_scope="M"
    )
    services.agent_types.add_knowledge(
        project_id=project.id, type_id=t.id, actor_id=owner.id, kind="fact", content="temp fact"
    )
    ...
    records = services.agent_types._repo.list_knowledge_for_type(t.id, include_archived=True)
    assert len(records) == 1
    assert records[0].state == "archived"


def test_retire_blocked_when_instances_running(services, project_with_owner) -> None:
    ...
    services.agent_types.assign_instance_to_task(
        project_id=project.id, instance_id=inst.id, task_id=task.id, actor_id=owner.id
    )
...
def test_cross_project_knowledge_access_returns_nothing(services) -> None:
    ...
    b_records = services.agent_types.list_knowledge_for_project(project_b.id, actor_id=owner_b.id)
