...
def test_store_artifact_returns_hash(services, project_with_owner) -> None:
    ...
    art = services.artifacts.store_artifact(
        ...
        kind="stdout", content="hello world",
        producer="test",
    )
...
def test_artifact_hash_round_trip(services, project_with_owner) -> None:
    ...
    art = services.artifacts.store_artifact(
        project_id=project.id, actor_id=owner.id,
        kind="test_report", content=content,
    )
...
def test_artifact_handle_is_bounded(services, project_with_owner) -> None:
    ...
    art = services.artifacts.store_artifact(
        project_id=project.id, actor_id=owner.id,
        kind="stdout", content=long_content,
    )
...
def test_ingest_and_search_rag(services, project_with_owner) -> None:
    ...
    services.artifacts.ingest_rag_document(
        project_id=project.id, actor_id=owner.id,
        source_type="manual", source_id="doc2",
    ...
    results = services.artifacts.search_rag(
        project_id=project.id, query="authentication"
    )
...
def test_cross_project_rag_returns_zero(services) -> None:
    ...
    services.artifacts.ingest_rag_document(
        project_id=project_a.id, actor_id=owner_a.id,
        source_type="manual", source_id="secret1",
    ...
    results = services.artifacts.search_rag(
        project_id=project_b.id, query="zebra elephant"
    )
...
def test_rebuild_index_reproduces_content(services, project_with_owner) -> None:
    ...
    results_before = services.artifacts.search_rag(
        project_id=project.id, query="pineapple"
    )
...
def test_candidate_not_searchable_until_approved(
    ...
    results = services.artifacts.search_rag(
        project_id=project.id, query="dragonfly"
    )
    assert len(results) == 0
    # Approve it.
    doc = services.artifacts.list_rag_documents(
        project.id, state="candidate"
    )[0]
    services.artifacts.approve_rag_document(
        project_id=project.id, doc_id=doc.id, actor_id=owner.id
    )
...
def test_supersede_document(services, project_with_owner) -> None:
    ...
    old_doc = services.artifacts.ingest_rag_document(
        project_id=project.id, actor_id=owner.id,
        source_type="manual", source_id="v1",
        title="Design v1",
    ...
    new_doc = services.artifacts.ingest_rag_document(
        project_id=project.id, actor_id=owner.id,
        source_type="manual", source_id="v2",
        title="Design v2",
    ...
    results = services.artifacts.search_rag(
        project_id=project.id, query="REST"
    )
    assert len(results) == 0
    # The new doc is searchable.
    results = services.artifacts.search_rag(
        project_id=project.id, query="GraphQL"
    )
    assert len(results) == 1
...
def test_artifact_list_by_kind(services, project_with_owner) -> None:
    ...
    services.artifacts.store_artifact(
        project_id=project.id, actor_id=owner.id,
        kind="stdout", content="output 1",
    )
    services.artifacts.store_artifact(
        project_id=project.id, actor_id=owner.id,
        kind="stderr", content="error 1",
    )
    stdout_only = services.artifacts.list_artifacts(
        project_id=project.id, kind="stdout"
    )
