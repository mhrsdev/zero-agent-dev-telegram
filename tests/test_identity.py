...
def test_create_user_records_audit_event(services) -> None:
    ...
    assert len(events) >= 1
    create_event = next((e for e in events if e.operation == "user.create"), None)
...
def test_create_project_automatically_adds_owner_membership(services) -> None:
    ...
    owner = services.identity.create_user(display_name="Owner")
    project = services.identity.create_project(owner_id=owner.id, name="Project A")
    membership = services.identity._identity_repo.get_membership(project.id, owner.id)
...
def test_create_project_records_audit_event(services) -> None:
    ...
    create_event = next((e for e in events if e.operation == "project.create"), None)
    assert create_event is not None
...
def test_create_project_rejects_nonexistent_owner(services) -> None:
    with pytest.raises(UserNotFoundError):
        services.identity.create_project(owner_id=UserId("zu_nonexistent"), name="Project X")
...
def test_verify_external_identity_enables_resolution(services) -> None:
    ...
    services.identity.verify_external_identity(platform="telegram", external_id="7086634092")
