...
class PlanRepository:
    ...
    def insert_conversation_event(
        self, event: ConversationEvent, *, commit: bool = True
    ) -> None:
    ...
    def get_conversation_event(
        self, event_id: ConversationEventId
    ) -> ConversationEvent:
        conn = self._database.connect()
        cursor = conn.execute(
        ...
        row = cursor.fetchone()
        if row is None:
            raise ConversationEventNotFoundError(
                f"Conversation event {event_id} not found"
            )
    ...
    def insert_plan(
        self, plan: Plan, *, commit: bool = True
    ) -> None:
        conn = self._database.connect()
        try:
            conn.execute(
    ...
    def insert_revision(
        self, revision: PlanRevision, *, commit: bool = True
    ) -> None:
        conn = self._database.connect()
        try:
            conn.execute(
                ...
                (
                    ...
                    json.dumps(
                        [eid.value for eid in revision.content.source_event_ids]
                    ),
    ...
    def get_revision(
        ...
        if row is None:
            raise PlanRevisionNotFoundError(
                f"Plan revision {revision_id} not found"
            )
    ...
    def get_current_revision(self, plan_id: PlanId) -> PlanRevision:
        ...
        if plan.current_revision_number == 0:
            raise PlanRevisionNotFoundError(
                f"Plan {plan_id} has no revisions"
            )
        conn = self._database.connect()
        cursor = conn.execute(
            "SELECT id, plan_id, project_id, revision_number, objective, "
            ...
            (plan_id.value, plan.current_revision_number),
        )
        row = cursor.fetchone()
        if row is None:
            raise PlanRevisionNotFoundError(
    ...
    def insert_approval(
        self, approval: PlanApproval, *, commit: bool = True
    ) -> None:
        conn = self._database.connect()
        try:
            conn.execute(
    ...
    def insert_handoff(
        self, handoff: PlanHandoff, *, commit: bool = True
    ) -> None:
    ...
    def set_handoff_execution_id(
        ...
        *,
        ...
        conn = self._database.connect()
        cursor = conn.execute(
            "UPDATE plan_handoffs SET execution_id = ? WHERE id = ? "
