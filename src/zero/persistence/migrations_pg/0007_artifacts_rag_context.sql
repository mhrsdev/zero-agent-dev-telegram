-- GENERATED from 0007_artifacts_rag_context.sql by scripts/gen_pg_migrations.py.
...
CREATE OR REPLACE FUNCTION zero_artifacts_no_update_fn() RETURNS trigger AS $zero$
BEGIN
    RAISE EXCEPTION 'artifacts is append-only';
END;
$zero$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS artifacts_no_update ON artifacts;
CREATE TRIGGER artifacts_no_update
    BEFORE UPDATE ON artifacts
    FOR EACH ROW EXECUTE FUNCTION zero_artifacts_no_update_fn();

CREATE OR REPLACE FUNCTION zero_artifacts_no_delete_fn() RETURNS trigger AS $zero$
BEGIN
    RAISE EXCEPTION 'artifacts is append-only';
END;
$zero$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS artifacts_no_delete ON artifacts;
CREATE TRIGGER artifacts_no_delete
    BEFORE DELETE ON artifacts
    FOR EACH ROW EXECUTE FUNCTION zero_artifacts_no_delete_fn();
